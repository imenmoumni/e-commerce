...
class Settings {
	...
	/*public static function get_settings_data_clean( $post_id, $section ) {

		if ( empty( self::$data[ $section ] ) ) {
			self::$data[ $section ] = array();
		}

		if ( ! isset( self::$data[ $section ][ $post_id ] ) ) {
			self::set_settings_data( $post_id, $section );
		}
		...
		return self::$data[ $section ][ $post_id ];
	}*/
	 ...
	 * @since    1.0.0
	 */
	public static function get_settings_data( $post_id, $sections = [] ) {
		...
		return isset( self::$data[ $post_id ] ) ? self::$data[ $post_id ] : [];
	}
	...
	public static function get_taxonomies() {
		...
		foreach ( $wp_taxonomies as $taxonomy ) {
			...
			//array_push( $taxonomies, $taxonomy_data );
		}
		return $taxonomies;
	}
	...
	public static function get_template_data( $post_id ) {
		...
		if ( $settings['instances'] && $settings['instance_order'] && $settings['template'] ) {
			return $settings;
		}
	...
	public static function get_option( $option_name ) {
		if ( $option_name === 'breakpoints' ) {
			return get_option('custom_layouts_breakpoints', array(
				// 'large' => '1920',
				'medium' => '1280',
				'small' => '960',
				'xsmall' => '600',
