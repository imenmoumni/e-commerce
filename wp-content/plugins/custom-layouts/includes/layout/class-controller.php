...
class Controller {
	...
	private function get_device_type_attribute_value( $device_size, $setting_prefix ) {
		...
		if ( $active_device_index === false ) {
			return '';
		} else if ( $active_device_index === $device_count - 1 ) {
			return $this->settings[ $active_value_index ];
		}
	...
	public function render( $return = false ){
		...
		//if ( 'inline' === CSS_Loader::get_mode() ) {
			$spacing_class = 'cl-layout--item-spacing-' . $this->settings['item_spacing'];
			$this->add_class( $spacing_class );
			echo '<style>.'.esc_attr( $spacing_class ).' .cl-layout__item-spacing { padding: ' . esc_html( $this->settings['item_spacing'] ). 'px; }</style>';
		//}
		

		$container_class = $this->settings['container_class'] !== '' ? ' ' . $this->settings['container_class'] : '';
		echo '<div class="cl-layout-container'. esc_attr( $container_class ) .'">';
				...
				if ( count( $query_data[ 'ids' ] ) > 0 ) {
					foreach ( $query_data[ 'ids' ] as $result_id ) {
						...
						echo '<div '. Util::get_attributes_html( $attributes ) . '>';
							...
							echo '<div class="cl-layout__item-spacing">';
					...
					echo wp_kses_post( '<div class="cl-layout__no-results">' . $this->settings[ 'no_results_message' ] . '</div>' );
				}
			}
		}
