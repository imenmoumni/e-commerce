...
class Controller {
	...
	private function init( $id ) {
		...
		$this->id = $id;
		$this->settings = Settings::get_template_data( $id );
	 ...
	 * functions like `get_the_permalink()` continue to work in varioush hooks like the
	 *  ones attached to `read more`
	 */
	...
	private function get_image_postion_class(){
		...
		if ( $show_featured_image === 'yes' ) {
			...
			$image_positions = [ 'top', 'right', 'bottom', 'left' ];
