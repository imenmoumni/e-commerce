...
class Title extends Element_Base {
	...
	public function render( $post, $instance, $template, $return = false ) {
		...
		if ( $link_to_post === 'yes' ) {
			$output = '<a class="cl-element-title__anchor" href="' . esc_url( get_permalink( $post->ID ) ) . '">' . esc_html( $post_title ). '</a>';
		} else {
	...
	public function get_css( $instance, $template_class, $template = array() ) {
		...
		$link_styles = $this->take_array_elements( array(
			'fontFamily', 'fontFormatBold', 'fontFormatItalic', 'fontFormatUnderline', 'fontSize'
		), $instance_data );
		...
		$hover_settings = $this->take_array_elements( array(
			'fontFormatBoldHover', 'fontFormatItalicHover', 'fontFormatUnderlineHover'
		), $instance_data );
		$hover_styles = array(
