...
class Featured_Media extends Element_Base {
	...
	public function render( $post, $instance, $template, $return = false ) {
		...
		if ( $image_url === '' ) {
			...
			} else if ( $featured_image_axis === 'horizontal' ) {
		...
		if ( 'yes' === $link_to_post ) {
			$output = '<a class="cl-element-featured_media__anchor" href="' . esc_url( get_permalink( $post->ID ) ) . '">' . $output . '</a>';
		}
	...
	public function get_css( $instance, $template_class, $template = array() ) {
		...
		} else if( $container_sizing === 'aspect_ratio' ) {
			if ( $featured_image_axis === 'vertical' ) {
		...
		if ( ! $use_img_tag ) {
			...
			$image_styles = wp_parse_args( array(
